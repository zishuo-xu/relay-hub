...
RelayHub 是本仓库中的独立简历项目。它参考 Clowder AI 的产品问题和架构经验，但不复制其源码，也不追求复刻全部功能。项目重点是从零实现一条可解释、可测试、可演示的多 Agent 协作主链路。
...
RelayHub 的代码、包名、数据模型和 UI 都在本目录内独立实现。根项目只作为研究材料；实现过程中通过公开接口思想和行为需求进行重新设计，避免复制 Clowder AI 的内部实现。

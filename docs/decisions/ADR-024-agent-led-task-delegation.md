...
- 状态：Superseded by ADR-025（第一实现切片保留为受控的隔离工作能力）
- 日期：2026-08-14
...
2026-08-15 起，RelayHub 的主协作结构改为 ADR-025 的消息路由与责任接力：默认在同一 Thread 内由当前负责人明确交给下一位 Agent，而不是先创建父子 Task 树。这里的 Delegation 保留给确实需要独立、长期执行面的少量工作，并不再承担日常团队协作。

...
8. [ ] 从现有 Task/Run/Handoff/Review/Event 完整推导 State、Owner、Evidence、Verdict、Route；当前页面已先展示责任 Agent、Handoff 状态和下一动作。
...
Phase 3.4 的第一切片已完成：封闭 `NextAction`、版本化 Handoff V2、内容摘要校验和 `handoff.consumed` 已接入现有 Builder → Reviewer 主链。下一步先完成统一 Route/责任查询投影，让 Review、返工、等待用户和完成在页面与 API 中使用同一组 State/Owner/Evidence/Verdict/Route 语义；之后才开放方案、UX、安全等专业 Agent 的动态 Handoff。暂不建设任意 Workflow DAG；Phase 4 的 lease/reconciliation 完成后，才评估并行 fan-out 和外部信号等待。

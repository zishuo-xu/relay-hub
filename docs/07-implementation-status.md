...
- Thread Composer 明确区分“分别回答”和“协作完成”；前者保持一条消息多 Task 的独立回答语义。
- 协作模式要求一个 Lead 和至少一名协作者，只创建一个 Lead Task，并持久化允许协作者列表。
- Lead 使用既有受控 Consultation 分配有限问题；平台限制可路由 Agent，咨询后自动恢复 Lead 评估并综合。
- 页面展示主导 Agent、团队规模和单一协作执行卡，不再把同题并发误称为自动分工。

### 当前边界

- 协作者串行执行，最多三次 Consultation；尚未实现并行 Assignment、投票或通用 DAG。

## 2026-08-14：对话输入区 Agent 选择器

### 已实现

- 对话输入区移除受操作系统样式影响的原生 Agent `select`，改为 RelayHub 自有的紧凑浮层选择器。
- 候选项同时展示 Agent 名称、运行 CLI 和模型，已选择 Agent 继续以可移除 chip 展示；选择后自动收起，并支持点击外部或 `Escape` 关闭。
- 浮层限制高度并内部滚动，保持最多 4 个并行 Agent 的既有约束，不改变 Message Dispatch、Task 或 Run 合约。
...
- API `handoff.requested` 按 `nextAction.type` 分流：两类路由都要求来源是 Task 当前 running Run；`request_review` 只能指向 Task 固定 Reviewer，`handoff` 还校验目标存在、启用、同 Workspace、非自身。目标校验失败、历史或非当前 Run 均确定性抛错，不产生 Handoff、Run 或 Outbox。
...
- Contracts 33/33、Worker 47/47 单元测试通过，覆盖合法/非法信封、目标一致性、旧 RunOutcome 兼容、三个 Adapter 的通用 Handoff与固定 Reviewer 回退、权限边界和 Mock 链式路由。
...
- Architect 复验使用全新隔离数据库 `relayhub_arch_verify_20260812`，API 43/43 通过；全仓 `git diff --check` 与 `pnpm check` 通过（类型检查、全部单元测试、Next.js production build）。
- Mock 浏览器 Task `46885794-24f7-423a-a1ac-d7eb6ac8adcd` 完成 A → B → Mock Reviewer → `waiting_for_user`；Timeline 36 条事件，Agent 身份以“名称（短 ID）”展示。614 × 772 视口无页面级溢出，控制台无 warning/error。
- 真实 OpenCode Task `93a04c9f-f320-492f-889b-634975e1de6e` 使用两个独立 `opencode-go/gpt-5.6-luna` Agent 完成 A → B；两个 Run 均 succeeded，Handoff `034a2b40-c58e-4ee0-ada7-ce63ead19432` accepted，数据库可重建 parent、trigger 和身份快照，Task 按策略进入 `waiting_for_user`。
- 真实模型曾返回 malformed、错误嵌套、字符串 refs 和额外字段；严格 Schema 均确定性拒绝。Prompt 已补完整 sibling 信封示例、2,000 字符边界与引用对象约束，最终真实链通过，未用宽松解析掩盖协议错误。

## 2026-08-10：Agent 编辑与任务筛选一致性

### 已实现

- Agent 编辑抽屉在发起 CLI/模型目录检测前同步回填所选 Agent，避免加载期间显示“新建 Agent”或上一个 Profile。
- 每次打开抽屉生成递增请求序号；关闭或切换 Agent 后，过期运行时响应不再更新当前表单。加载期间保存按钮明确显示并保持禁用。
- 当前任务进入 completed/cancelled 等终态时，“待处理”自动切换为“全部”，不再把终态任务混入待处理列表。
- 用户从终态任务主动点击“待处理”时，页面选择第一条需要关注的任务；筛选计数、可见项目数和当前详情保持一致。

### 验证证据

- Web 新增 7 个纯逻辑回归测试，覆盖 Agent 同步回填、过期请求拒绝、新 Agent 默认模型、终态筛选切换和待处理重新选择。
- 全仓 `pnpm check` 通过，包含类型检查、全部单元测试和 Next.js production build。
- `http://localhost:3010/` 浏览器快速执行 OpenCode 编辑 → 关闭 → Codex 编辑；第一帧和 1.5 秒后均保持 `Codex Builder`，没有旧响应覆盖。
- 已完成任务被选择时“全部 28”保持激活；点击“待处理 5”后自动选择待确认任务，侧栏严格显示 5 项。614 × 772 视口无页面级溢出，控制台无 warning/error。
...
- 官方和自定义 ProviderConnection 均可从设置列表点击管理；官方连接管理名称与启停，自定义连接还可编辑协议、Base URI、凭证环境变量名称和模型目录。
- 连接类型与运行 CLI 创建后不可变。服务端阻止停用仍被启用 Agent 使用的连接，也阻止移除启用 Agent 正在使用的模型。
- 默认“检测配置”检查 CLI、Worker 凭证环境变量和模型目录，不发送模型请求；自定义连接在用户勾选费用确认后可执行真实调用测试。
- 真实调用测试只通过 OpenCode 在临时空目录中向第一个配置模型发送固定 `RELAYHUB_OK` 文本，禁用工具与分享，结束后清理临时目录，并明确标记可能产生 provider 用量。
- 连接保存后刷新 Workspace 配置；Agent 模型下拉继续直接读取连接的最新模型目录，历史 Run 的连接快照保持不变。

### 验证证据

- Contracts 20/20、Worker 14/14 测试通过；隔离 PostgreSQL API 测试 19/19 通过，包含连接引用与 Run 快照集成用例。
- 全仓 `pnpm check` 通过，覆盖 TypeScript、单元测试及所有生产构建。
- 真实 `http://localhost:3010/` 验收通过：官方连接可点击管理，启用 Agent 使用时停用选项被禁用并显示影响数量；无费用配置检测返回健康；新增自定义连接表单完整可用。未保存正式连接，也未执行真实付费调用，浏览器控制台无错误。
